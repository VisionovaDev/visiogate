...
@section('content')
    <div class="container">
        <h3 class="fw-bold mb-3 mt-3">Modifica di: {{ $persona->nome }} {{ $persona->cognome }}</h3>
        <form action="{{ route('imposta.persona.update', $persona->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-4 form-group mt-3 {{ $errors->has('nome') ? 'has-error' : '' }}">
                    ...
                    <input type="text" class="form-control" id="nome" name="nome"
                        value="{{ old('nome', $persona->nome) }}" required>
                    @if ($errors->has('nome'))
                ...
                <div class="col-md-4 form-group mt-3 {{ $errors->has('cognome') ? 'has-error' : '' }}">
                    ...
                    <input type="text" class="form-control" id="cognome" name="cognome"
                        value="{{ old('cognome', $persona->cognome) }}" required>
            ...
            <div class="row">
                <div class="col-md-4 form-group mt-3 {{ $errors->has('interno') ? 'has-error' : '' }}">
                    ...
                    <input type="text" class="form-control" id="interno" name="interno"
                        value="{{ old('interno', $persona->interno) }}">
                ...
                <div class="col-md-4 form-group mt-3 {{ $errors->has('reparto_id') ? 'has-error' : '' }}">
                    <div class="mb-3">
                        ...
                        <select class="form-select form-control" id="reparto_id" name="reparto_id">
                            @foreach ($persona->reparto->sede->reparti as $reparto_sede)
                                <option value="{{ $reparto_sede->id }}"  @if ($reparto_sede->id == $persona->reparto->id ) SELECTED  @endif >
                    ...
                    <div class="mb-3 mt-3">
                        <div class="form-check form-switch">
                            ...
                            <input class="form-check-input" type="checkbox" role="switch" id="accetta_visite"
                                ...
                                {{ old('accetta_visite', $persona->accetta_visite) ? 'checked' : '' }}>
