...
<div class="container">
    ...
    <form action="{{ route('imposta.sede.update', ['id' => $sede->id]) }}" method="POST">
        ...
        <div class="form-group mt-33 {{ $errors->has('nome') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="nome" name="nome" value="{{ old('nome', $sede->nome) }}" required>
            @if ($errors->has('nome'))
        ...
        <div class="form-group mt-33 {{ $errors->has('indirizzo') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="indirizzo" name="indirizzo" value="{{ old('indirizzo', $sede->indirizzo) }}" required>
        ...
        <div class="form-group mt-33 {{ $errors->has('citta') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="citta" name="citta" value="{{ old('citta', $sede->citta) }}" required>
            @if ($errors->has('citta'))
        ...
        <div class="form-group mt-33 {{ $errors->has('cap') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="cap" name="cap" value="{{ old('cap', $sede->cap) }}" required>
            @if ($errors->has('cap'))
        ...
        <div class="form-group mt-33 {{ $errors->has('provincia') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="provincia" name="provincia" maxlength="2" value="{{ old('provincia', $sede->provincia) }}" required>
        ...
        <div class="form-group mt-33 {{ $errors->has('nazione') ? 'has-error' : '' }}">
            ...
            <input type="text" class="form-control" id="nazione" name="nazione" value="{{ old('nazione', $sede->nazione) }}" required>
        ...
        <div class="mb-3">        
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="switchEmailEntrata" name="is_email_entrata_abilitata" value="{{ $sede->is_email_entrata_abilitata }}">
                <label class="form-check-label" for="switchEmailEntrata">Invia email all'entrata TODO</label>
            </div>
        </div>
       <!-- Fine Toggle per is_email_entrata_abilitata -->
        ...
        <div class="mb-10">
            <ul class="nav nav-tabs mt-3" id="msgTabEntrata" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link{{ $loop->first ? ' active' : '' }}" id="msgTabEntrata_{{ $lang }}-tab" data-bs-toggle="tab" data-bs-target="#msgTabEntrata_{{ $lang }}" type="button" role="tab" aria-controls="{{ $lang }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}"><span class="fi fi-{{ $lang }}"></span>&nbsp;{{ $label }}</button>
                </li>
                @endforeach
            </ul>
            <div class="tab-content mt-3" id="msgTabEntrataTabContent">
                @foreach (['it', 'en', 'de', 'fr', 'es'] as $lang)
                <div class="tab-pane overflow-auto fade{{ $loop->first ? ' show active' : '' }}" style="max-height: 500px;" id="msgTabEntrata_{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                    <textarea rows=10 class="form-control" id="msg_email_entrata_{{ $lang }}" name="msg_email_entrata_{{ $lang }}" required>
        ...
        {{-- inizio gestione email in uscita --}}
          ...
          <div class="mb-3">        
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="switchEmailuscita" name="is_email_uscita_abilitata" value="{{ $sede->is_email_uscita_abilitata }}">
                <label class="form-check-label" for="switchEmailuscita">Invia email all'uscita TODO</label>
            </div>
        </div>
       <!-- Fine Toggle per is_email_uscita_abilitata -->
        ...
        <div class="mb-10">
            <ul class="nav nav-tabs mt-3" id="msgTabUscita" role="tablist">
                @foreach (['it' => 'Italiano', 'en' => 'Inglese', 'de' => 'Tedesco', 'fr' => 'Francese', 'es' => 'Spagnolo'] as $lang => $label)
                <li class="nav-item" role="presentation">
                    <button class="nav-link{{ $loop->first ? ' active' : '' }}" id="msgTabUscita_{{ $lang }}-tab" data-bs-toggle="tab" data-bs-target="#msgTabUscita_{{ $lang }}" type="button" role="tab" aria-controls="{{ $lang }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}"><span class="fi fi-{{ $lang }}"></span>&nbsp;{{ $label }}</button>
                </li>
                @endforeach
            </ul>
            <div class="tab-content mt-3" id="msgTabUscitaTabContent">
                @foreach (['it', 'en', 'de', 'fr', 'es'] as $lang)
                <div class="tab-pane overflow-auto fade{{ $loop->first ? ' show active' : '' }}" style="max-height: 500px;" id="msgTabUscita_{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                    <textarea rows=10 class="form-control" id="msg_email_uscita_{{ $lang }}" name="msg_email_uscita_{{ $lang }}" required>
        ...
        <div class="mb-10">
            <ul class="nav nav-tabs mt-3" id="msgTabRegolamento" role="tablist">
                @foreach (['it' => 'Italiano', 'en' => 'Inglese', 'de' => 'Tedesco', 'fr' => 'Francese', 'es' => 'Spagnolo'] as $lang => $label)
                <li class="nav-item" role="presentation">
                    <button class="nav-link{{ $loop->first ? ' active' : '' }}" id="msgTabRegolamento_{{ $lang }}-tab" data-bs-toggle="tab" data-bs-target="#msgTabRegolamento_{{ $lang }}" type="button" role="tab" aria-controls="{{ $lang }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}"><span class="fi fi-{{ $lang }}"></span>&nbsp;{{ $label }}</button>
                </li>
                @endforeach
            </ul>
            <div class="tab-content mt-3" id="msgTabRegolamentoTabContent">
                @foreach (['it', 'en', 'de', 'fr', 'es'] as $lang)
                <div class="tab-pane overflow-auto fade{{ $loop->first ? ' show active' : '' }}" style="max-height: 500px;" id="msgTabRegolamento_{{ $lang }}" role="tabpanel" aria-labelledby="{{ $lang }}-tab">
                    <textarea rows=10 class="form-control" id="regolamento_{{ $lang }}" name="regolamento_{{ $lang }}" required>
